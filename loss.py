...
def G_logistic_renyi(G, D, opt, training_set, minibatch_size, alpha=0.8): # pylint: disable=unused-argument
    #print(alpha)
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = training_set.get_random_labels_tf(minibatch_size)
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    fake_scores_out = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
    loss = tf.ones_like(fake_scores_out) * (1.0 / (alpha - 1.0) * tf.log(tf.reduce_mean(tf.pow(1 - tf.sigmoid(fake_scores_out), (alpha - 1.0) * tf.ones_like(fake_scores_out)))))
    return loss
