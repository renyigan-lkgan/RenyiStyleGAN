...
if 1:
    ...
    D             = EasyDict(func_name='training.networks_stylegan.D_basic', mbstd_group_size=1)               # Options for discriminator network.
    ...
    D_loss        = EasyDict(func_name='training.loss.D_logistic_simplegp', r1_gamma=0.0) # Options for discriminator loss.
    ...
    desc += '-celeba';     dataset = EasyDict(tfrecord_dir='celeba-renyi', resolution=64);                 train.mirror_augment = False
    ...
    desc += '-4gpu'; submit_config.num_gpus = 4; sched.minibatch_base = 16; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 128, 128: 16}
    #desc += '-8gpu'; submit_config.num_gpus = 8; sched.minibatch_base = 32; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 32}
    # StyleGAN Loss
    #desc += '-stylegan'; G_loss        = EasyDict(func_name='training.loss.G_logistic_nonsaturating')           # Options for generator loss.
    # Renyi Loss
    alpha = 3.0
    desc += f"-renyi_{alpha}"; G_loss        = EasyDict(func_name='training.loss.G_logistic_renyi', alpha=alpha)           # Options for generator loss.
    # Use gradient penalty and minibatch deviation layer..
    #desc += '-mbstd-r1pen'; D.mbstd_group_size = 4; D_loss.r1_gamma=10.0
    # Seed Used
    seed_used = 3000
    desc += f"-seed_{seed_used}"; tf_config     = {'rnd.np_random_seed': seed_used}                                           # Options for tflib.init_tf().
    # Default options.
